...
class Player:
    ...
    def process_people(self, index_choice: int, choices: List[PotentialPartner]):
        self.happiness += choices[index_choice].happiness
    ...
    def process_misfortune(self, choices: List[event.Misfortune]):
        ...
        if choice.side_effects == event.Side_Effect.NONE:
            ...
            objects_to_remove = [obj for obj in self.investments if obj.type_investment == event.Type_Investment.STOCK]
            ...
            objects_to_remove = [obj for obj in self.investments if obj.type_investment == event.Type_Investment.LAND]
            if len(objects_to_remove):
                if objects_to_remove:
                    self.investments.remove(random.choice(objects_to_remove))
        elif choice.side_effects == event.Side_Effect.LOSE_REAL_ESTATE:
            objects_to_remove = [obj for obj in self.investments if obj.type_investment == event.Type_Investment.REAL_ESTATE]
            if len(objects_to_remove):
                if objects_to_remove:
                    self.investments.remove(random.choice(objects_to_remove))
    ...
    def process_choice(self, current_choices: dict, index_choice: int):
        ...
        if type_choice == "education":
            ...
            self.process_investment(index_choice, json_array_to_investment_list(choices))
...
def json_array_to_item_list(json_array: List[dict]) -> List[event.Item]:
    ...
    for item in json_array:
        item_list.append(event.Item(item['name'], item['description'], item['image'], item['happiness'], item['money']))
    return item_list

def json_array_to_investment_list(json_array: List[dict]) -> List[event.Investment]:
    investment_list = []
    for item in json_array:
        investment_list.append(event.Investment(item['name'], item['description'], item['base_price'], item['type_investment']))
    return investment_list

def json_array_to_misfortune_list(json_array: List[dict]) -> List[event.Misfortune]:
    misfortune_list = []
    for item in json_array:
        misfortune_list.append(event.Misfortune(item['name'], item['description'], item['happiness'], item['money'], item['side_effects']))
    return misfortune_list

def json_array_to_special_event_list(json_array: List[dict]) -> List[event.Special_Event]:
    special_event_list = []
    for item in json_array:
        special_event_list.append(event.Special_Event(item['name'], item['description'], item['happiness'], item['money'], item['requirements']))
    return special_event_list

def json_array_to_job_list(json_array: List[dict]) -> List[event.Job]:
    job_list = []
    for item in json_array:
        job_list.append(event.Job(item['name'], item['base_salary'], item['required_qualification']))
    return job_list

def json_array_to_people_list(json_array: List[dict]) -> List[PotentialPartner]:
    people_list = []
    for item in json_array:
        people_list.append(PotentialPartner(item['name'], item['gender'], item['description']))
    return people_list